...
CVMREZ=$(onetemplate instantiate "debian9"  --name client-vm  --user luch4001 --password $password  --endpoint $CENDPOINT)
...
git clone git@github.com:chlukas/VirtualizacijuFailai.git

touch dbIP.txt
echo $db_IP > dbIP.txt

sed -i '9s|.*|GRANT ALL ON virtualizacijos.* TO 'root'@'$web_IP' IDENTIFIED BY 'root';|' my_ansible/DB_Data.sql
